...
void training_process(double *layer_one, double *layer_two, double *layer_three, 
                     ...
                     double expected[]);
void write_weights(string model_weights, double **layer_one_weights, double **layer_two_weights);
...
int main()
{
  ...
  for (int image_index = 0; image_index < 60000; image_index++)
  {
    ...
    // update weights every x number of iterations to save cpu cycles
  	if (image_index % 50 == 0)
	  {
...
void write_weights(string model_weights, double **layer_one_weights, double **layer_two_weights)
{
  ofstream training_weights;
  training_weights.open(model_weights.c_str(), ios::out);
  cout << "upating weights to " << model_weights << " file" << endl;
