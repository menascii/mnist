...
void training_process(double *layer_one, double *layer_two, double *layer_three, 
                     ...
                     double &cost_error, double expected[]);
...
int main()
{
  ...
  // mnist binary training digits                                       
  string training_image_fn = "train-images";
  // mnist binary training labels                                                                                    
  string training_label_fn = "train-labels";
  // output weights file name                                                           
  string model_fn = "model-weights";
  
  // number of training images
  int mnist_image_count = 60000;
  ...
  // sample prediction cost error value
  double cost_error;
  ...
  // initialize neural network partial derivatives
  layer_two_thetas = new double [128];
  layer_three_thetas = new double [10];

  // training loop to iterate each mnist digit and label
  for (int image_index = 0; image_index < mnist_image_count; image_index++)
    {
      cout << "mnist training image #: " << image_index << endl; 
      // read image and label
      // assign each 28*28 pixel to first layer of 784 activations in neural network
      read_image(image, label, image_digit, layer_one, expected);
      training_process(layer_one, layer_two, layer_three, 
    	...
    	if (image_index % 500 == 0)
    	  {
    	 	    write_weights(model_fn, layer_one_weights, layer_two_weights);
    	  }
  }
  ...
  for (int i = 0; i < input_layer_size; i++)
    {
      ...
      for (int j = 0; j < output_layer_size; j++)
        {
          int sign = rand() % 2;
          layer_weights[i][j] = (double)(rand() % 7) / 10.0;
          if (sign == 1)
            {
...
{ 
  ...
  // deltas use the difference between target activation and current activation to train model
  for (int i = 0; i < 10; i++)
    {
...
void write_weights(string file_name, double **layer_one_weights, double **layer_two_weights)
{
  ...
  cout << "saving the updated weights for neural network to " << file_name << " file" << endl;
