...
int main()
{
  ...
  image.open(training_images.c_str(), ios::in | ios::binary);
  // open mnist training labels
  label.open(training_labels.c_str(), ios::in | ios::binary);
...
{ 
  ...
  // deltas use the change between target activation and current activation to train model
  // update layer one and layer two weights
  for (int i = 0; i < 10; i++)
    {
